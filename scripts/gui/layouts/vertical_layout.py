...
class VerticalLayout(Container):
    ...
    def set_children_position(self) -> None:
        content_size = pygame.Vector2(0, -self.__padding)
        children = self.get_children(True)

        for child in children:
            content_size.x = max(child.get_width(), content_size.x)
            content_size.y += child.get_height()
        ...
        if self.__justify_content == JustifyContent.PADDING:
            ...
            self._rect = pygame.FRect((0, 0), (content_size.x, self.__length))
            amount = len(children)
            if self.__justify_content == JustifyContent.SPACE_BETWEEN:
                if amount > 1:
                    padding = (self.__length - content_size.y) / (len(children) - 1)
                else:
                    padding = 0
            else:
                padding = (self.__length - content_size.y) / (len(children) + 1)
        ...
        if self.__justify_content == JustifyContent.SPACE_EVENLY:
            midtop = pygame.Vector2(self._rect.width / 2, padding)
        else:
            midtop = pygame.Vector2(self._rect.width / 2, 0)

        for child in self.get_children(True):
            child.move_to(midtop, Anchors.TOP)
            midtop += (0, child.get_height() + padding)
